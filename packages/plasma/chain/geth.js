...
const { plasmaContractArtifacts, providerUrl } = require('./config')
...
const init = (contractAddress, operatorAddress) => {
  plasmaContract = new web3.eth.Contract(
    plasmaContractArtifacts.abi,
    contractAddress,
    {
      gas: 1000000
    }
...
const startWithdrawal = async (
  ...
  let result = await plasmaContract.methods
...
const challengeWithdrawal = async (
  withdrawalId,
  blkNum,
  txIndex,
  oIndex,
  ...
  let result = await plasmaContract.methods
    .challengeWithdrawal(withdrawalId, blkNum, txIndex, oIndex, targetTx, proof)
    .send({
      from: from,
      gas: 300000
    })
...
const getWithdrawals = async blockNumber => {
  let withdrawalEvents = await plasmaContract.getPastEvents(
