...
const initHttpServer = http_port => {
  ...
  app.post('/deposit', async (req, res) => {
    try {
      const result = await geth.deposit(req.body.address, req.body.amount)
      res.send(result)
    } catch (err) {
      res.send(JSON.stringify(err))
    }
  ...
  app.post('/withdraw/create', async (req, res) => {
    try {
      const p = block.getTransactionProofInBlock(
        req.body.blkNum,
        req.body.txIndex
      )
      const withdrawalId = await geth.startWithdrawal(
        req.body.blkNum,
        req.body.txIndex,
        req.body.oIndex,
        p.tx,
  ...
  app.post('/withdraw/challenge', async (req, res) => {
    try {
      ...
      await geth.challengeWithdrawal(
  ...
  app.listen(http_port, () =>
    console.log('Aureum Plasma RPC on port: ' + http_port)
  )
}

const main = () => {
  const argv = minimist(process.argv.slice(2), {
    string: ['port', 'contract', 'operator']
  })
