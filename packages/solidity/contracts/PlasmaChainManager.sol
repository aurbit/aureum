...
contract PlasmaChainManager {
    ...
    event WithdrawalCompleteEvent(
        uint256 indexed blockNumber,
        uint256 exitBlockNumber,
        uint256 exitTxIndex,
        uint256 exitOIndex
    );
    ...
    function isValidProof(
        ...
        for (uint256 i = 32; i < proof.length; i += 33) {
            bytes1 flag;
            bytes32 sibling;
            assembly {
        ...
        returns (bool spent)
    {
        ...
        for (uint256 i = 0; i < 2; i++) {
            if (!txList[3 * i].isEmpty()) {
                ...
                uint256 txIndex = txList[3 * i + 1].isEmpty()
                    ? 0
                    : txList[3 * i + 1].toUint();
                uint256 oIndex = txList[3 * i + 2].isEmpty()
                    ? 0
                    : txList[3 * i + 2].toUint();
                if (
                    record.blockNumber == blockNumber &&
