...
contract('PlasmaChainManager', function (accounts) {
  it('deposit', async () => {
    ...
    try {
      await request(
        'POST',
        '/deposit',
        JSON.stringify({ address: accounts[0], amount: 0.2 })
      )
  ...
  it('transact', async () => {
    ...
    try {
      await request(
        'POST',
        '/deposit',
        JSON.stringify({ address: accounts[1], amount: 0.5 })
      )
      await request('POST', '/mineBlock')
      await request(
        'POST',
        '/transact',
        JSON.stringify({ from: accounts[1], to: accounts[2], amount: 0.3 })
      )
      await request('POST', '/mineBlock')
      var utxo = await request('GET', '/utxo')
  ...
  it('withdraw', async () => {
    ...
    try {
      ...
      var withdrawId = await request(
        'POST',
        '/withdraw/create',
        JSON.stringify({
          from: accounts[1],
          blkNum: utxo[0].blkNum,
          txIndex: utxo[0].txIndex,
          oIndex: utxo[0].oIndex
        })
      )
      await sleep(1000)
      await request(
        'POST',
        '/withdraw/finalize',
        JSON.stringify({ from: accounts[1] })
      )
      await request('POST', '/mineBlock')
